(function() {
    ...
    var pusher = new Pusher('66f2adfe0cb37ef56ced', {
    ...
    let helpers = {
        ...
        LogIntoChatSession: function (evt) {
            ...
            // Disable the form
            chatBody.find('#loginScreenForm input, #loginScreenForm buttonID').attr('disabled', true)
            console.log('test log 2', chat)
            if ((name !== '' && name.length >= 3) && (email !== '' && email.length >= 5)) {
                console.log('test log 1', chat)
                axios.post('/new/guest', {name, email}).then(response => {
                    chat.name = name;
                    chat.email = email;
                    console.log('test log 3', chat);
